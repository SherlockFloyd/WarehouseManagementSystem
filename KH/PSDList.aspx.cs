...
public partial class Admin_PSDList : System.Web.UI.Page
{
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select * from  V_配货 where 配送单号 like '%" + TextBox1.Text + "%'  and 货物名称 like '%" + TextBox2.Text + "%' and 名称 like '%" + TextBox3.Text + "%' and 客户帐号='" + Session["UserID"] + "' ");
        if (dt != null)
        {
            this.gvData.DataKeyNames = new string[] { "配送单号" };
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "PSD.aspx?ID=" + gvData.DataKeys[num]["配送单号"].ToString();
            PageBase.OpenWebForm(strUrl);
        }
        else if (e.CommandName == "Del")
        {
            DataControl.Execute("delete from 配送单 where 配送单号='" + gvData.DataKeys[num]["配送单号"].ToString() + "'");
            BindResult();
        }
    }
